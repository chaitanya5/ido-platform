...
contract IdoInvestmentsPresale {
    ...
    address payable internal idoFactoryAddress; // address that creates the presale contracts
    address payable public idoDevAddress; // address where dev fees will be transferred to
    address public idoLiqLockAddress; // address where LP tokens will be locked
    ...
    uint256 private idoDevFeePercentage; // dev fee to support the development of Ido Investments
    uint256 private idoMinDevFeeInWei; // minimum fixed dev fee to support the development of Ido Investments
    uint256 public idoId; // used for fetching presale without referencing its address
    ...
    bool public idoDevFeesExempted = false; // if true, presale will be exempted from dev fees
    ...
    constructor(address _idoFactoryAddress, address _idoDevAddress) public {
        require(_idoFactoryAddress != address(0));
        require(_idoDevAddress != address(0));

        idoFactoryAddress = payable(_idoFactoryAddress);
        idoDevAddress = payable(_idoDevAddress);
    }

    modifier onlyIdoDev() {
        require(idoFactoryAddress == msg.sender || idoDevAddress == msg.sender);
        _;
    }

    modifier onlyIdoFactory() {
        require(idoFactoryAddress == msg.sender);
        _;
    }

    modifier onlyPresaleCreatorOrIdoFactory() {
        require(
            presaleCreatorAddress == msg.sender || idoFactoryAddress == msg.sender,
    ...
    ) external onlyPresaleCreatorOrIdoFactory {
    ...
    function setIdoInfo(
        address _idoLiqLockAddress,
        uint256 _idoDevFeePercentage,
        uint256 _idoMinDevFeeInWei,
        uint256 _idoId
    ) external onlyIdoDev {
        idoLiqLockAddress = _idoLiqLockAddress;
        idoDevFeePercentage = _idoDevFeePercentage;
        idoMinDevFeeInWei = _idoMinDevFeeInWei;
        idoId = _idoId;
    }

    function setIdoDevFeesExempted(bool _idoDevFeesExempted)
    external
    onlyIdoDev
    {
        idoDevFeesExempted = _idoDevFeesExempted;
    }

    function setOnlyWhitelistedAddressesAllowed(bool _onlyWhitelistedAddressesAllowed)
    external
    onlyPresaleCreatorOrIdoFactory
    {
    ...
    function addLiquidityAndLockLPTokens() external presaleIsNotCancelled {
        ...
        uint256 idoDevFeeInWei;
        if (!idoDevFeesExempted) {
            uint256 pctDevFee = finalTotalCollectedWei.mul(idoDevFeePercentage).div(100);
            idoDevFeeInWei = pctDevFee > idoMinDevFeeInWei || idoMinDevFeeInWei >= finalTotalCollectedWei
            ? pctDevFee
            : idoMinDevFeeInWei;
        }
        if (idoDevFeeInWei > 0) {
            finalTotalCollectedWei = finalTotalCollectedWei.sub(idoDevFeeInWei);
            idoDevAddress.transfer(idoDevFeeInWei);
        }
    ...
    function cancelAndTransferTokensToPresaleCreator() external {
        if (!uniLiquidityAdded && presaleCreatorAddress != msg.sender && idoDevAddress != msg.sender) {
            revert();
        }
        if (uniLiquidityAdded && idoDevAddress != msg.sender) {
            revert();
        }
