...
contract IdoInvestmentsFactory {
    ...
    event PresaleCreated(bytes32 title, uint256 idoId, address creator);
    ...
    IdoInvestmentsInfo public immutable IdoInfo;
    //IERC20 public platformToken; // Platform token

    constructor(address _idoInfoAddress) public {
        IdoInfo = IdoInvestmentsInfo(_idoInfoAddress);
    ...
    function initializePresale(
        IdoInvestmentsPresale _presale,
    ...
    function createPresale(
        ...
        //IERC20 platformToken = IERC20(0x9E09E11Ae96Bc3Aa14D21C92bb40ed477012D675);

        IdoInvestmentsPresale presale = new IdoInvestmentsPresale(address(this), IdoInfo.owner());
        ...
        IdoInvestmentsLiquidityLock liquidityLock = new IdoInvestmentsLiquidityLock(
                IERC20(pairAddress),
                msg.sender,
                _uniInfo.liquidityAddingTime + (_uniInfo.lpTokensLockDurationInDays * 1 days)
            );

        uint256 idoId = IdoInfo.addPresaleAddress(address(presale));
        presale.setIdoInfo(address(liquidityLock), IdoInfo.getDevFeePercentage(), IdoInfo.getMinDevFeeInWei(), idoId);

        //presale.setPlatformTokenAddress(address(platformToken));

        emit PresaleCreated(_stringInfo.saleTitle, idoId, msg.sender);
    }
}
